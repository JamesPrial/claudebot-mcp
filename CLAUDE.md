...
**Entry point:** `cmd/claudebot-mcp/main.go` — 14-step startup sequence, graceful shutdown on SIGINT/SIGTERM.

**Tool packages** (`internal/{message,reaction,channel,guild,user}/`): Each exports a `FunctionTools()` returning `[]tools.Registration`. Tools are registered in `main.go` via `tools.RegisterAll()`.
